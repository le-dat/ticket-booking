...
Module path: `github.com/zentrix-app/zentrix-backend-go-temp`. The Go version is declared in `go.mod`.
...
`README.md` (English) is canonical. `README_VN.md` (Vietnamese) is a full translation that
mirror it **section for section, table row for table row** — same headings in the same order, same
tables, same code blocks, same links. Changing one and not the other is a common
regression in this repo.

Update both whenever a change touches what they document:
