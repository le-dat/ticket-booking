...
logger = logging.getLogger("payment-service.exceptions")
...
async def http_exception_handler(request: Request, exc: StarletteHTTPException) -> JSONResponse:
    ...
    if isinstance(detail, dict):
        ...
        if "details" in detail and isinstance(detail["details"], list):
            details = [
                ErrorItem(**d) if isinstance(d, dict) else ErrorItem(message=str(d))
...
async def unhandled_exception_handler(request: Request, exc: Exception) -> JSONResponse:
    """Handle unexpected 500 server crashes and log error."""
